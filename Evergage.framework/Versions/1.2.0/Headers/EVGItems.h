...
/**
 Abstract base class. An item is something in a site or app that users can view or otherwise engage with. Typical examples are products,
 articles and blogs. Evergage tracks descriptive fields and user activity against items. This activity can be used to
 build reports and to promote relevant items to users.
 
 All fields are optional unless otherwise stated. Generally, optional fields should be sent on `<[EVGContext viewItem:]>`
 calls if their values are known.
...
- (nonnull instancetype)init __attribute__((deprecated("Use class factory methods")));
...
/**
 A product is an is an item that can be sold to users. Products can be added to `<EVGLineItem>` objects
...
/**
 Builds a product, including many of the commonly used fields. This method only builds a data object. To use it, pass it
 to one of the tracking methods like `<[EVGContext viewItem:]>`.
 
 @param evgId The ID of the product. This is the only required field.
...
/**
 * Date and time when this article was first published.
 */
@property (nullable, nonatomic, strong) NSDate *publishedDate;
...
/**
 * Date and time when this blog was first published.
 */
@property (nullable, nonatomic, strong) NSDate *publishedDate;
...
/**
 ...
 The only required field is the lineItems array of `<EVGLineItem>`s.
...
/**
 Required array of `<EVGLineItem>`s ordered by the user.
 */
@property (nonnull, nonatomic, strong) NSArray<EVGLineItem *> *lineItems;
...
/**
 ...
 @param lineItems The required items being ordered along with their quantities. All entries must be of type `<EVGLineItem>`.
...
+ (nonnull instancetype)orderWithId:(nullable NSString *)orderId
                          lineItems:(nonnull NSArray<EVGLineItem *> *)lineItems
